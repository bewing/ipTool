...
/**
 * RESTful Class for handling IP allocations.
 * Used with https://github.com/jacwright/RestServer/blob/master/RestServer.php
 * to provide a JSON REST frontend to the class.
 *
 * @access public
 * @category Net
 * @package  ipTool
...
class ipToolRest
{
    ...
    private function __construct()
    {
        $dsn = array("phptype"          => "mysql",
                     ...
                     "username"         => "ipTool",
                     "password"         => "bTmnZalQrD");
        $store = MDB2::singleton($dsn, array("portability" => MDB2_PORTABILITY_ALL ^ MDB2_PORTABILITY_FIX_CASE));
        if (PEAR::isError($store)) {
            die($store->getMessage());
        }
        $this->db = $store;
    }
     ...
     * @url GET /netblock/$netblockId
     */
    public function getNetblock($netblockId = null)
    {
     ...
     * @access public
     * @return array|MDB2_Error array of netblockIds on success, an MDB2 error on failure
     * @url GET /netblock/$parentId/children
     */
     ...
     * @url POST /netblock/$parentId/$bitmask/$objectId/$desiredNetwork
     */
    public function assignNetblock($objectId, $parentId, $bitmask, $desiredNetwork = NULL)
    {
     ...
     * @url DELETE /netblock/$netblockId
     */
     ...
     * @access public
     * @return Net_IPv4Alloc|MDB2_Error Net_IPv4Alloc on success, an MDB2 error on failure
     * @url GET /netblock/$netblockId/full
     */
     ...
     * @url PUT /netblock/$netblockId/$objectId
     */
